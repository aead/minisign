...
ifneq ($(shell git status -s) , )
	...
	@GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -trimpath -ldflags "-s -w -X 'main.version=${VERSION}'" -o ./minisign ./cmd/minisign
	@tar -czf minisign-linux-amd64.tar.gz ./minisign ./LICENSE ./README.md
	@echo " DONE."

	@echo -n Building minisign ${VERSION} for linux/arm64...
	@GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -trimpath -ldflags "-s -w -X 'main.version=${VERSION}'" -o ./minisign ./cmd/minisign
	@tar -czf minisign-linux-arm64.tar.gz ./minisign ./LICENSE ./README.md
	@echo " DONE."

	@echo -n Building minisign ${VERSION} for darwin/arm64...
	@GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 go build -trimpath -ldflags "-s -w -X 'main.version=${VERSION}'" -o ./minisign ./cmd/minisign
	@tar -czf minisign-darwin-arm64.tar.gz ./minisign ./LICENSE ./README.md
	@echo " DONE."

	@echo -n Building minisign ${VERSION} for windows/amd64...
	@GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -trimpath -ldflags "-s -w -X 'main.version=${VERSION}'" -o ./minisign ./cmd/minisign
